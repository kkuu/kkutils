 ...
 */
public abstract class KKStickyHeaderDecoration extends RecyclerView.ItemDecoration {
    ...
    /***
     * 用kk adapter 的时候 用true
     */
    private boolean isImmersion;

    private int gravity = Gravity.LEFT;

    public KKStickyHeaderDecoration(RecyclerView.Adapter adapter) {
        this(adapter, true);
    }

    public KKStickyHeaderDecoration(RecyclerView.Adapter adapter, boolean isImmersion) {
        mAdapter = adapter;
    ...
    public abstract boolean hasHeader(int position) ;
    public abstract void headerChange(View currHead) ;
    ...
    private RecyclerView.ViewHolder getHeader(RecyclerView parent, int position) {
        ...
        if (mHeaderCache.containsKey(key)) {
            ...
            RecyclerView.ViewHolder viewHolder = mAdapter.onCreateViewHolder(parent, mAdapter.getItemViewType(position));
            final View header = viewHolder.itemView;

            mAdapter.onBindViewHolder(viewHolder,position);

            if(header.getLayoutParams()==null){
                header.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
            }
            ...
            mHeaderCache.put(key, viewHolder);
        ...
        for (int layoutPos = 0; layoutPos < childCount; layoutPos++) {
            ...
            if (adapterPos != RecyclerView.NO_POSITION) {
                if (layoutPos == 0) {
                    ...
                    View header = getHeader(parent, headerPos).itemView;
                    drawHead(c,parent,header,layoutPos);
    ...
    View currHead;
    public void drawHead(Canvas c, RecyclerView parent, View header, int layoutPos){
        final View child = parent.getChildAt(layoutPos);
        final int adapterPos = parent.getChildAdapterPosition(child);
