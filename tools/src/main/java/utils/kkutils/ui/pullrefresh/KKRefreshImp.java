package utils.kkutils.ui.pullrefresh;//package utils.gfreeutils.ui;//package utils.aixiangutils.ui;//package utils.wzutils.ui.pullrefresh;
...
public class KKRefreshImp extends KKRefresh {
    ...
    public KKRefreshImp(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    public KKRefreshImp(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public static void init(int layout_header, int res_tv_refresh, int res_progressBar) {
        KKRefreshImp.layout_header = layout_header;
        KKRefreshImp.res_tv_refresh = res_tv_refresh;
        KKRefreshImp.res_progressBar = res_progressBar;
    }
    ...
    public void initHeaderAndFooter() {
        final KKRefresh KKRefresh = this;
        if (useHeader&&wz_recycle_header==null) {//不判断空，会导致如果设置了 footer 为null  那么这里也会重新执行， 就会导致 没有measure 显示不出来， 下面footer一样
            ...
            KKRefresh.setHeaderView(wz_recycle_header);
        }
        ...
        if (useFooter&&wz_recycle_footer==null) {
            ...
            KKRefresh.setFooterView(wz_recycle_footer);
        }


        KKRefresh.setRefreshListener(new RefreshListener() {
            @Override
            ...
            public void onPullProgressChanged(int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
                ...
                if (scrollY < 0) {
                    if (wz_recycle_header != null) {
                        if (KKRefresh.isReadyToRefresh()) {
                            String tem = "释放刷新";
                            if (!tem.equals(txt_header)) {
                                setHeaderText(tem);
                            }
                        } else if (KKRefresh.isReadyToBackBegin()) {
                            String tem = "下拉刷新";
                            if (!tem.equals(txt_header)) {
                                setHeaderText(tem);
                            }
